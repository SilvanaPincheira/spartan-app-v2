...
// GET /api/business-evaluations?customer_id=&limit=20&page=1
export async function GET(req: Request) {
  try {
    ...
    let q = supabase.from("business_evaluations").select("*", { count: "exact" }).order("created_at", { ascending: false }).range(from, to);
...
export async function POST(req: Request) {
  try {
    ...
    if (!body.customer_id || !body.evaluation_result) {
      return NextResponse.json({ data:null, error: "customer_id y evaluation_result son obligatorios" }, { status: 400 });
    }
    const payload = {
      customer_id: body.customer_id,
      evaluation_result: body.evaluation_result, // enum eval_status
