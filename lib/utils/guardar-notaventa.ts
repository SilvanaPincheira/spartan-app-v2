export async function guardarPdfYEnviarSinEstado({
  ...
  try {
    ...
    const respuestaTexto = await resSave.text();

if (!resSave.ok) {
let mensajeError = "Error al guardar en Google Sheets.";
...
try {
  ...
  console.error(
    "Respuesta no válida de /api/save-to-sheets:",
    respuestaTexto
  );
}
...
try {
  ...
  console.warn(
    "La nota fue guardada, pero la respuesta no era JSON:",
    respuestaTexto
  );
}
...
if (resultado?.error || resultado?.success === false) {
  throw new Error(
    resultado.error ||
    resultado.message ||
    "Google Sheets informó un error."
  );
}
}
